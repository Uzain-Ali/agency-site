...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_FILES['image']['name'])) {
        $target_dir = $_SERVER['DOCUMENT_ROOT'] . "/agency-site/uploads/services/";
    ...
    if ($isEdit) {
        ...
        if ($stmt->execute()) {
            header("Location: ../admin/services.php?added=1");
            exit;
        } else {
...
<head>
  <meta charset="UTF-8">
  <title><?= $isEdit ? 'Edit Service' : 'Add Service' ?></title>
  ...
  <div class="container-fluid">
    <div class="row flex-nowrap">
      ...
      <main class="col-lg-10 col-md-9 ms-sm-auto px-4 py-4 main-content">
        <h1 class="mb-4 text-center"><?= $isEdit ? 'Edit Service' : 'Add Service' ?></h1>
        <?php if ($error): ?>
        ...
        <div class="d-flex justify-content-center align-items-center" style="min-height:60vh;">
          <form method="post" enctype="multipart/form-data" class="bg-black p-4 rounded shadow-sm w-100" style="max-width:500px;">
            ...
            <?php endif; ?>
            <div class="mb-3">
              <label class="form-label" for="title"><i class="bi bi-gear"></i> Title</label>
            ...
            <div class="mb-3">
              <label class="form-label" for="description"><i class="bi bi-file-text"></i> Description</label>
            ...
            <div class="mb-3">
              <label class="form-label" for="price_basic"><i class="bi bi-currency-dollar"></i> Basic Price</label>
              <input type="number" step="0.01" class="form-control" id="price_basic" name="price_basic" required value="<?= htmlspecialchars($service['price_basic']) ?>">
            </div>
            <div class="mb-3">
              <label class="form-label" for="price_premium"><i class="bi bi-currency-dollar"></i> Premium Price</label>
              <input type="number" step="0.01" class="form-control" id="price_premium" name="price_premium" required value="<?= htmlspecialchars($service['price_premium']) ?>">
            </div>
            <div class="mb-3">
              ...
              <?php if ($isEdit && $service['image']): ?>
                <div class="mt-2">
                  <img src="../public/assets/services/<?= htmlspecialchars($service['image']) ?>" alt="Service Image" style="width:80px;">
                </div>
              <?php endif; ?>
            </div>
            <button type="submit" class="btn btn-gold w-100"><?= $isEdit ? 'Update Service' : 'Add Service' ?></button>
          </form>
        </div>
      </main>
    </div>
